...
define(['jquery', 'TYPO3/CMS/Imagemap/Fabric'], function ($, fabric) {
      ...
      value: function initializeEvents() {
        ...
        this.getElements('.positionOptions .t3js-field').forEach(function (field) {
          field.addEventListener('keyup', function (event) {
            ...
            that.eventDelay = AreaFormElement.wait(function () {
  ...
  function (_Aggregation) {
    ...
    function Rect() {
      ...
      _this3 = _possibleConstructorReturn(this, (_getPrototypeOf4 = _getPrototypeOf(Rect)).call.apply(_getPrototypeOf4, [this].concat(args)));

      _defineProperty(_assertThisInitialized(_this3), "name", 'rect');
  ...
  function (_Aggregation2) {
    ...
    function Circle() {
      ...
      _this5 = _possibleConstructorReturn(this, (_getPrototypeOf5 = _getPrototypeOf(Circle)).call.apply(_getPrototypeOf5, [this].concat(args)));

      _defineProperty(_assertThisInitialized(_this5), "name", 'circle');
  ...
  function (_Aggregation3) {
    ...
    function Poly() {
      ...
      _this7 = _possibleConstructorReturn(this, (_getPrototypeOf6 = _getPrototypeOf(Poly)).call.apply(_getPrototypeOf6, [this].concat(args)));

      _defineProperty(_assertThisInitialized(_this7), "name", 'poly');

      _defineProperty(_assertThisInitialized(_this7), "controls", []);
    ...
    _createClass(Poly, [{
      ...
      value: function updateFields() {
        ...
        this.getElement('.link').value = this.link;
        Object.entries(this.attributes).forEach(function (attribute) {
          _this8.getElement('#' + attribute[0]).value = attribute[1];
        });
        var parentElement = this.getElement('.positionOptions');
        this.points.forEach(function (point, index) {
          point.id = point.id ? point.id : 'p' + _this8.id + '_' + index;

          if (!point.hasOwnProperty('element')) {
            point.element = _this8.getFormElement('#polyCoords', point.id);
      ...
      value: function addControls(areaConfig) {
        ...
        this.points.forEach(function (point, index) {
          _this9.addControl(areaConfig, point, index);
        });
        ...
        if (this.points.length > 3) {
          ...
          this.points.forEach(function (point, index) {
            if (element.id !== point.id) {
              points.push(point);
              controls.push(_this10.controls[index]);
            } else {
              point.element.remove();

              _this10.canvas.remove(_this10.controls[index]);
            }
          });
          points.forEach(function (point, index) {
            var oldId = point.id;
            point.id = 'p' + _this10.id + '_' + index;
            _this10.getElement('#' + oldId).id = point.id;
            _this10.getElement('#x' + oldId).id = 'x' + point.id;
            _this10.getElement('#y' + oldId).id = 'y' + point.id;

            _this10.getElement('[for="x' + oldId + '"]').setAttribute('for', 'x' + point.id);

            _this10.getElement('[for="y' + oldId + '"]').setAttribute('for', 'y' + point.id);
    ...
    _createClass(AreaEditor, [{
      ...
      value: function initializeAreas(areas) {
        ...
        areas.forEach(function (area) {
          switch (area.shape) {
            case 'rect':
              _this11.addRect(area);

              break;

            case 'circle':
              _this11.addCircle(area);

              break;

            case 'poly':
              _this11.addPoly(area);
